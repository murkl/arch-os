...
**Note:** _The build copies whatever is in `modules/`, so **[Create boot medium](../modules/imager)** is on the image as well — but it is never offered there. Its `requires.sh` says this is not a machine for writing a device, so the page that asks what to open has the other two on it and nothing else._
