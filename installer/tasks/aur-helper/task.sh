...
#
# In the user's own configuration rather than in /etc/paru.conf: that file
# belongs to the paru package, so editing it would leave a .pacnew to merge on
# every paru update — and a preference about how one person is asked for a
# password is theirs to change, not the system's.
