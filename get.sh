...
and writes it to a USB device, so this machine can make the one that boots it.
  ...
  DEBUG=true            touch no hardware: no device written, a simulated run
...
install_mode() {
    ...
    # oak.yaml beside its own binary and asks which to open. Answers and logs
    # land there too, so running this again picks up where it left off.
    #
