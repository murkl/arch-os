...
main() {
 ...
 ${COLOR_RESET}"
    ...
    # Prepare properties
    set_properties_default   # Set default properties
    source_properties_file   # Load properties file and auto export variables
    generate_properties_file # Generate & source properties
    source_properties_file   # Source generated properties
    ...
    # Check properties
    check_properties && print_info "Properties successfully initialized"
...
select_username() {
    ...
    generate_properties_file && print_info "Username is set to ${ARCH_OS_USERNAME}"
}
...
select_password() {
    ...
    generate_properties_file && print_info "Password is set to *********"
}
...
install() {
    ...
    # Ask for password
    if [ -z "${ARCH_OS_PASSWORD}" ]; then
        print_input "Enter Arch OS Password and hit enter (may not visible):" && read -r -s input_password </dev/tty && echo
        [ -z "$input_password" ] && print_error "Password is null" && exit 1
        print_input "Enter Arch OS Password again and hit enter (may not visible):" && read -r -s input_password_check </dev/tty && echo
    ...
    if [ "$input_check" = "y" ] || [ "$input_check" = "Y" ]; then
        ...
        echo -e " •••••••••••••••••••••••••••••••••••••••••••••••••••••"
        cat "$SCRIPT_CONF" # Print properties file to stdout
        echo -e " •••••••••••••••••••••••••••••••••••••••••••••••••••••"
