# The answers, put where another machine can pick them up - see import.sh
# beside this file for the other end.
#
# Only ever the answer file: a user name, a host name, a disk, a language. The
# password is not in it, because Oak never writes a secret down. Nothing here
# runs unasked either: the task offers itself first and opens on no, and it is
# the only thing in this module that sends anything anywhere.
#
...
# paste.rs takes a file over an ordinary POST, answers with the address it
# now lives at, and serves it back as plain text. No account, no key.
service="https://paste.rs"

# An answer, appended to the file Oak keeps them in as one KEY='value' line and
# reads back. Any earlier line for the same name is dropped, and the file is
...
# survives the machine being restarted. See the copy-config task.
