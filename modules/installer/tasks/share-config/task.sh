...
#
# An installation is two dozen answers, and setting up the next machine the
# same way is otherwise those answers typed in again by hand. So the
# finished configuration can be put where a phone reaches it, and a starting
# point can take its answers from there - see import.sh beside this file for
# the other end.
#
...
# Kept as an answer of its own, which is what puts it on the page the run
# stops on next.
answer ARCH_OS_CONFIG_URL "$url"

# And into the copy already in the new system - see the copy-config task:
# the one record of it that survives the machine being restarted.
